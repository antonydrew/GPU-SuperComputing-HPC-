...
#define PATH "/home/xtreme/Projects/DemoMono/"			
...
void main (int argc, char *argv[]){  
	...
	int *intperiod,c, lens=0, combos=0,counters=0,starto=0,tachy=0;
	long *dt; 
	double *op,*hi,*lo,*p,*price, *smooth, *detrender, *period, *qu, *iu, *ji, *jq, *ib, *qb, *sib, *sqb,*re, *im,*sre,*sim,*speriod,*smperiod,*qc,*ic,*ric,*sig,*nois,*snr,*pos,*pnl, *cumpnl, *sharp;
	double *rets, *zscores,*sumi, *vari, *stdevi, *m_avei,*sumv, *varv, *stdevv, *m_avev, *dolls, *cumdolls, *sumip, *varip, *stdevip, *m_aveip, *sharpp;
	int i=0,combo=0,ii=0,zz=0,wins=0,counter=0,start=1,stop=0,startf=0,stopf=0,beg=1,high=0,m=0,mm=0,gg=0; char desty[50],dest[50],desta[50],tmp[50],strs[50],foldr[50],fnum[50],fnums[50],dir[50]; int peri[100] = { { 0 } };
 ...
 for (ii = 1; ii <= peri[gg] ; ii++) {	// loop is for periodicity - so 30yrs of price data divided into sub-units for rolling optimization (aka parameter sweeps)
  for (z = 0; z < lensa; z++) {			// 2 nested for loops for parameter sweep or combination of arrays a[] and b[]//
   for (j = 0; j < lensb; j++) {	
	...
	rets=(double*) calloc(end+1,sizeof(double));
	dolls=(double*) calloc(end+1,sizeof(double));
	cumdolls=(double*) calloc(end+1,sizeof(double));
	pos=(double*) calloc(end+1,sizeof(double));
	pnl=(double*) calloc(end+1,sizeof(double));
	cumpnl=(double*) calloc(end+1,sizeof(double));

	zscores=(double*) calloc(end+1,sizeof(double));
...
//INSERT NEW STUFF HERE EACH TIME START SUB-FUNCTION*************///CALLING function//STEP C//
	...
	for(i=start;i<stop;i++){														//IN-sample rolling optimization
		
		if(/*snr[i] > b[j] && */zscores[i] > a[z]) pos[i] = 1.00;														
		if(/*snr[i] > b[j] && */zscores[i] < -a[z]) pos[i] = -1.00;			
		pnl[i] = (pos[i] * rets[i])-TRANS;  }
		
		
		//sharpe(pnl, sumi, vari, end, stdevi, m_avei, sharp);
	...
	for(i=start;i<stop;i++) {
		fprintf(ferr,"%ld\t %10.6lf\t %10.3lf\t %10.2lf\t %10.4lf\t %10.5lf\t %10.5lf\t %10.5lf\n",dt[i],p[i],zscores[i],pos[i],dolls[i],rets[i],pnl[i],sharpp[i]); } //	
...
//ADD IN EACH POINTER VARIABLE HERE - FREEING UP SPACE IN MEMORY*******STEP D//
	...
	free(rets);free(pos);free(pnl);free(zscores);free(cumpnl);free(op);free(sharp);free(sumi);free(vari);free(stdevi);free(m_avei);
	free(sumv);free(varv);free(stdevv);free(m_avev);free(dolls);free(cumdolls);free(sharpp);free(sumip);free(varip);free(stdevip);free(m_aveip);
	...
	for(i=starto;i<stopf;i++){														//OUT-OF-SAMPLE runs
	...
	for(i=starto;i<stopf;i++) {
		fprintf(ferr,"%ld\t %10.6lf\t %10.3lf\t %10.2lf\t %10.4lf\t %10.5lf\t %10.5lf\t %10.5lf\n",dt[i],p[i],zscores[i],pos[i],dolls[i],rets[i],pnl[i],sharpp[i]);
		fprintf(ferri,"%ld\t %10.6lf\t %10.3lf\t %10.2lf\t %10.4lf\t %10.5lf\t %10.5lf\t %10.5lf\n",dt[i],p[i],zscores[i],pos[i],dolls[i],rets[i],pnl[i],sharpp[i]+ocum[gg][2][ii-1]); } //	
	...
	t2 = clock();   
	diff = (((float)t2 - (float)t1) / 1000000.0F ) * 1;   
	...
	fprintf(recon,"\n\n%.2f sec..Avg I-MaxSharpe is %.2f..+I-Sharpes are %d of %d combos or %.0f%%..\n+%d Win Markets out of %i..Avg All I-Sharpes is %.2f\n\n",diff/60.0,avv/NUMI,combo,counter,(((double)(combo)/(double)(counter))*100),wins,NUMI,sharpie/(double)(counter)); 
