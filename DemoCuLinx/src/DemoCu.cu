...
#define PATH "/home/xtreme/cuda-workspace/DemoCu/"
...
#define NUMI 1				//up to 27 number of markets
...
__global__ void kernelSim(float *zscores_d,float *rets_d,float *pnl_d,float *pos_d,int start,int stop,float zcut,int lens){
	...
	//if(tid < stoppy){
		...
		if(opt >=lens && (pos_d[opt]==1.00 || pos_d[opt]==-1.00)) pnl_d[opt] = (pos_d[opt] * rets_d[opt]);
