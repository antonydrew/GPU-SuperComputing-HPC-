 ...
 for (ii = 1; ii <= peri[gg] ; ii++) {	// loop is for periodicity - so 30yrs of price data divided into sub-units for rolling optimization (aka parameter sweeps)
  for (z = 0; z < lensa; z++) {			// 2 nested for loops for parameter sweep or combination of arrays a[] and b[]//
   for (j = 0; j < lensb; j++) {	
	...
	zscores_d=(float*) calloc(end+1,sizeof(float));
	//cudaMemset(zscores_d, 0, (int)(end)*sizeof(float));
	pos_d=(float*) calloc(end+1,sizeof(float));
	//cudaMemset(pos_d, 0, (int)(end)*sizeof(float));
	pnl_d=(float*) calloc(end+1,sizeof(float));
	//cudaMemset(pnl_d, 0, (int)(end)*sizeof(float));
	rets_d=(float*) calloc(end+1,sizeof(float));
...
//ADD IN EACH POINTER VARIABLE HERE - FREEING UP SPACE IN MEMORY*******STEP D//
	...
	free(rets);free(zscores);free(cumpnl);free(op);free(sharp);free(sumi);free(vari);free(stdevi);free(m_avei);
	...
	free(zscores_d);free(pnl_d);free(pos_d);free(rets_d);free(pos);free(pnl);
	...
	//Start outer loop and declare DATA REGION - list pointers/arrays for copying to and from GPU
	#pragma acc data copyin(zscores,rets,starto,stopf) copyout(pos,pnl)
	{
		//Declare KERNEL for ACCELERATION - sub-loop
		#pragma acc kernels
		{	
			...
			//Start innermost loop to step thru values and rules to get positions and returns
			#pragma acc loop independent
			for(int opt = starto; opt < stopf; opt++){	
			zcut = (float)(sharplist[gg][1][ii-tachy]);
			if(zscores[opt] > zcut && opt >=lensz) pos[opt] = buy;														
			if(zscores[opt] < -zcut && opt >=lensz) pos[opt] = sell;			
			if(opt >=lensz && (pos[opt]==1.00 || pos[opt]==-1.00)) pnl[opt] = (pos[opt] * rets[opt]);
			else {pnl[opt] = flat; pos[opt] = flat; 
			}
		}
	}
	...
	free(hi);free(lo);free(p);free(price);free(smooth);free(detrender);free(period);free(qu);free(iu);free(ji);free(jq);free(ib);free(qb);free(sib);free(re);free(im);free(sre);free(sim);free(speriod);free(smperiod);free(qc);free(ic);free(ric);free(intperiod);free(sig);free(nois);free(snr);
	free(rets);free(zscores);free(cumpnl);free(op);free(sharp);free(sumi);free(vari);free(stdevi);free(m_avei);//cudaFree(zscores);cudaFree(rets);cudaFree(pnl);cudaFree(pos);
	free(sumv);free(varv);free(stdevv);free(m_avev);free(dolls);free(cumdolls);free(sharpp);free(sumip);free(varip);free(stdevip);free(m_aveip);free(pos);free(pnl);
	free(zscores_d);free(pnl_d);free(pos_d);free(rets_d);
